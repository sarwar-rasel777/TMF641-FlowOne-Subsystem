...
@Service("serviceOrderFallout")
public class ServiceOrderFallout extends ServiceOrderStateChangeEventService {

  @Autowired
  public ServiceOrderFallout(EventStateChangeEventTranslator eventTranslator, ValuesBuilderFactory valuesBuilderFactory, ObjectSerializer objectSerializer, ObjectMapper objectMapper, ObjectValidator objectValidator, @Lazy ReceivingSender receivingSender) {
  ...
  public boolean validateStateType(StateChangeEvent serviceOrder) {
    return Optional.ofNullable(serviceOrder).stream()
        .filter(event -> supportedState().equals(ServiceOrderStateType.fromValue(event.getState())))
        .anyMatch(event -> event.getPhaseNote() != null
            && event.getPhaseNote().getMessage() != null
            && event.getPhaseNote().getMessage().toLowerCase().contains("has entered in fallout"));
  }
