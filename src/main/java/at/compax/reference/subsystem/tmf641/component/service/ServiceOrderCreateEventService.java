...
import at.compax.reference.subsystem.tmf641.component.model.CreateEvent;
...
import at.compax.reference.subsystem.tmf641.component.model.ServiceOrderCreateEvent;
...
@Log4j2
@SubsystemPayload(translator = EventCreateEventTranslator.class)
public abstract class ServiceOrderCreateEventService implements SubsystemService, ReceivingService<Event>, ServiceOrderStateTypeValidator {

  public static final String CREATE_EVENT_TYPE = "ServiceOrderCreateEvent";

  private final EventCreateEventTranslator eventTranslator;
  ...
  protected ServiceOrderCreateEventService(EventCreateEventTranslator eventTranslator, ValuesBuilderFactory valuesBuilderFactory, ObjectSerializer objectSerializer, ObjectMapper objectMapper, ObjectValidator objectValidator,
      ReceivingSender receivingSender) {
  ...
  public boolean support(Event event) {
    ...
    if (!CREATE_EVENT_TYPE.equals(event.getEventType())) {
      return false;
    }
    CreateEvent serviceOrder;
    try {
      ServiceOrderCreateEvent serviceOrderCreateEvent = objectMapper.convertValue(event, ServiceOrderCreateEvent.class);
  ...
  private boolean validateStateType(CreateEvent serviceOrder) {
    return Optional.ofNullable(serviceOrder)
        .map(CreateEvent::getState)
