...
import at.compax.reference.subsystem.tmf641.component.translator.HubRequestTranslator;
import lombok.SneakyThrows;
import lombok.extern.log4j.Log4j2;

@Log4j2
@Service("registerListener")
public class RegisterListenerService extends AbstractService {

  @Autowired
  private HubRequestTranslator translator;

  @Override
  @SneakyThrows
  public PayloadCreationResponseModel createRequest(PayloadCreationModel model) {
    RegisterListener request = translator.translate(valuesConsumerFactory.consumer(model.getValues()));

    SubsystemRestModel<RegisterListener> restModel = new SubsystemRestModel<>();
  ...
  public PayloadSendingResponseModel sendRequest(PayloadSendingModel model) {
    log.info("Sending registerListener request...");
    try {
      ...
      log.error("Error while sending registerListener request.", e);
