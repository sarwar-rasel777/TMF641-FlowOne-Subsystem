...
@Log4j2
@Service("deleteListener")
public class DeleteListenerService extends AbstractService {
  ...
  public PayloadCreationResponseModel createRequest(PayloadCreationModel model) {
    ...
    restModel.setMethod(HttpMethod.DELETE.name());
  ...
  public PayloadSendingResponseModel sendRequest(PayloadSendingModel model) {
    log.info("Sending delete listener request...");
    try {
      ...
      log.error("Error while sending deleteListener request.", e);
